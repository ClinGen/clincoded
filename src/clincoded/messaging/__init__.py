...
def includeme(config):
    ...
    # ??? config.add_route('track-data-test', '/track-data-test')
...
# track data using snapshot
@view_config(route_name='track-data-test', request_method='GET')
def track_data_test(request):
    elasticsearch_server = 'http://localhost:9200/clincoded'
    return_object = {'status': 'Fail',
        'message': 'Unable to deliver track data'}
    ...
    # Check that data has expected elements
    try:
        #if resultJSON['_source']['embedded']['resourceType'] == 'classification':
        #    source = 'gci'
        #elif resultJSON['_source']['embedded']['@type'][0] == 'gdm':
        #    source = 'gdm'
        if resultJSON['_source']['embedded']['@type'][0] != 'gdm':
        #else:
            raise Exception
    ...
    try:
        #if source == 'gdm':
            message_template = deepcopy(clincoded.messaging.templates.gdm_track.message_template)
            # data_to_remove = clincoded.messaging.templates.gci_track.data_to_remove
            add_data_to_msg_template(resultJSON['_source']['embedded'], None, None, message_template)
            message = json.dumps(message_template, separators=(',', ':'))
        #else:
        #    message_template = deepcopy(clincoded.messaging.templates.gci_track.message_template)
            # data_to_remove = clincoded.messaging.templates.gci_track.data_to_remove
        #    add_data_to_msg_template(resultJSON['_source']['embedded'], None, None, message_template)
            # message = json.dumps(message_template, separators=(',', ':'))
