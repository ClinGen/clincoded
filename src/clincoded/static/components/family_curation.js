...
var FamilyCuration = createReactClass({
    ...
    handleCopyGroupPhenotypes(e) {
        ...
        if (associatedGroups && associatedGroups.length > 0) {
            const hpoIdsWithTerms = this.state.hpoWithTerms ? this.state.hpoWithTerms : associatedGroups.hpoIdInDiagnosis;
            if (hpoIdsWithTerms && hpoIdsWithTerms.length) {
                hpoIds = hpoIdsWithTerms.map((id) => {
                    return id;
                });
            } else {
    ...
    handleCopyGroupNotPhenotypes(e) {
        ...
        var hpoInElim = '';
        var hpoElimFreeText = '';
        const hpoInElimWithTerms = this.state.hpoInElimWithTerms ? this.state.hpoInElimWithTerms : associatedGroups.hpoIdInElimination;
        if (this.state.group) {
            // We have a group, so get the disease array from it.
            associatedGroups = [this.state.group];
        } else if (this.state.family && this.state.family.associatedGroups && this.state.family.associatedGroups.length) {
        ...
        if (associatedGroups && associatedGroups.length > 0) {
            if (hpoInElimWithTerms && hpoInElimWithTerms.length) {
                hpoInElim = hpoInElimWithTerms.map((id) => {
                    return id;
                });
            } else {
    ...
    componentDidUpdate(prevProps, prevState) {
        if ((this.state.group !== prevState.group) || (this.state.family !== prevState.family)) {
            ...
            if (groups && groups.length) {                
                groups.forEach(group => {
                    ...
                    if (group.hpoIdInElimination && group.hpoIdInElimination.length) {
                        let hpoIds = group.hpoIdInElimination;
