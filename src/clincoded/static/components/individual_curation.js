...
const IndividualCuration = createReactClass({
    ...
    createIndividual: function(individualDiseases, individualArticles, individualVariants, individualScores, hpoids, nothpoids) {
        ...
        if (hpoWithTerms) {
            newIndividual.hpoWithTerms = hpoWithTerms;
        } else if (newIndividual.hpoWithTerms) {
            delete newIndividual.hpoWithTerms;
        }
        ...
        } else if (newIndividual.hpoIdInElimination && newIndividual.hpoIdInElimination.length) {
            delete newIndividual.hpoIdInElimination;
        }
        const hpoInElimWithTerms = this.getFormValue('hpoelimwithterms').split(', ');
        if (hpoInElimWithTerms) {
            newIndividual.hpoInElimWithTerms = hpoInElimWithTerms;
        } else if (newIndividual.hpoInElimWithTerms) {
            delete newIndividual.hpoInElimWithTerms;
        }
...
function IndividualCommonDiseases() {
    ...
    let probandLabel = (individual && individual.proband ? <i className="icon icon-proband"></i> : null);
    let hpoWithTerms = individual && individual.hpoWithTerms ? individual.hpoWithTerms.join(', ') : '';
    let hpoInElimWithTerms = individual && individual.hpoInElimWithTerms ? individual.hpoInElimWithTerms.join(', ') : '';
    ...
    return (
        <div className="row">
            ...
            <Input type="textarea" ref="phenoterms" label={LabelPhenoTerms()} rows="2"
                ...
                labelClassName="col-sm-5 control-label" wrapperClassName="col-sm-7" groupClassName="form-group" />
            {associatedGroups && ((associatedGroups[0].hpoIdInDiagnosis && associatedGroups[0].hpoIdInDiagnosis.length) || associatedGroups[0].termsInDiagnosis) ?
                <Input type="button" ref="phenotypecopygroup" wrapperClassName="col-sm-7 col-sm-offset-5 orphane" inputClassName="btn-copy btn-last btn-sm" title="Copy Phenotype from Associated Group"
                        ...
                        <Panel title={LabelPanelTitleView(individual, 'Disease & Phenotype(s)')} panelClassName="panel-data">
                            <dl className="dl-horizontal">
                                ...
                                <div>
                                    ...
                                    <dd>{individual.hpoWithTerms.join(', ')}</dd>
                                </div>
                                ...
                                <div>
                                    ...
                                    <dd>{individual.hpoInElimWithTerms.join(', ')}</dd>
                                </div>
